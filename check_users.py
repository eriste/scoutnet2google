...
class GoogleUsersDirectory(object):
    ...
    def _compare_user(self, sn_user, g_user):
        ...
        return (g_user.first_name == sn_user.first_name
                and g_user.last_name == sn_user.last_name)

    def scoutnet_missing_in_google(self, scoutnet_users):
        """Look for Scoutnet users missing in Google."""
        result = set()
        for sn_user in scoutnet_users:
            matches = [g_user for g_user in self.all_users
                       if self._compare_user(sn_user, g_user)]
            if len(matches) == 0:
                result.add(sn_user)
        return list(result)

    def google_missing_in_scoutnet(self, scoutnet_users):
        """Look for Google users missing in Scoutnet."""
        result = set()
        for g_user in self.all_users:
            matches = [sn_user for sn_user in scoutnet_users
                       if self._compare_user(sn_user, g_user)]
            if len(matches) == 0:
    ...
    def get_all_users(self) -> object:
        ...
        def pick_mobile(data):
            ...
            mobiles = [phone['value'] for phone in data
        ...
        while True:
            result = self.service.users().list(domain=self.domain,
...
def print_sn_user(user, indent=2):
    ...
    print(
        '%s%-20.20s %-15.15s' % (' ' * indent, user.last_name, user.first_name))
...
def main() -> None:
    ...
    all_active_adults = [member for member in scoutnet.all_adults
                         if
                         member not in
                         scoutnet.all_unit_members('Övriga kårmedlemmar')]

    youth_units = config.getlist('scoutnet', 'youthgroup_with_accounts')
    logging.warning(
        'Adding members of "%s" as youth members.' % ('", "'.join(youth_units)))
    tmp = sum([scoutnet.all_unit_members(unit) for unit
                  in youth_units], [])
    youths = scoutnet.subtract_list(tmp, all_active_adults)
    print('Google: %d users, Scoutnet: %d users (%d adults, %d youths)'
          % (len(all_users),
    ...
    print('Scoutnet users missing in Google: %d'
          % len(scoutnet_missing_in_google))
    ...
    print('Google users missing in Scoutnet: %d'
          % len(google_missing_in_scoutnet))
